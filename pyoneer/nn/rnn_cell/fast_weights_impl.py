...
_FastWeightsStateTuple = collections.namedtuple("FastWeightsStateTuple",
                                                ("c", "a"))
  ...
  """
    ...
    def call(self, inputs, state, training=False):
        ...
        fast_weights = add(
            ...
            scalar_mul(
                self._fast_learning_rate,
                math_ops.matmul(
                    array_ops.transpose(hidden_state, [0, 2, 1]),
                    hidden_state)))

        h = array_ops.identity(hidden_state)
        inner = add(slow, math_ops.matmul(h, fast_weights))
        h = self._activation(
            normalization_ops.layer_norm(inner, self._inner_beta,
                                         self._inner_gamma)
...
class FastWeightsLSTMCell(rnn_cell_impl.LayerRNNCell):
  ...
  """
    ...
    def call(self, inputs, state, training=False):
        ...
        if self._use_layer_norm:
            gate_inputs = normalization_ops.layer_norm(
                gate_inputs, self._gate_beta, self._gate_gamma)
        ...
        fast_weights = add(
            ...
            scalar_mul(
                ...
                math_ops.matmul(
                    array_ops.transpose(expand_fast_j, [0, 2, 1]),
                    expand_fast_j)))
        ...
        new_c = add(
            multiply(c, sigmoid(add(f, self._forget_bias))),
            multiply(sigmoid(i), fast_j))

        if self._use_layer_norm:
            new_c = normalization_ops.layer_norm(new_c, self._c_beta,
                                                 self._c_gamma)
