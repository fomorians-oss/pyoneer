...
class NonLinearActionValue(tf.keras.Model):
    ...
    def __init__(self, state_normalizer, action_normalizer, scale=2.):
        super(NonLinearActionValue, self).__init__()
        self.state_normalizer = state_normalizer
        self.action_normalizer = action_normalizer
        kernel_initializer = tf.initializers.variance_scaling(scale=scale)
    ...
    def call(self, states, actions, training=False, reset_state=True):
        ...
        hidden0 = activation_ops.swish(self.bn0(hidden0, training=training))
        hidden1 = self.hidden1(tf.concat([hidden0, actions_norm], axis=-1))
        hidden1 = activation_ops.swish(self.bn1(hidden1, training=training))
