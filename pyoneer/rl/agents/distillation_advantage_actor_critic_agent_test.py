...
from pyoneer.rl.agents import distillation_advantage_actor_critic_agent_impl
...
class DistillationAdvantageActorCriticAgentTest(test.TestCase):

    def testComputeLoss(self):
        with context.eager_mode():
            agent = distillation_advantage_actor_critic_agent_impl.DistillationAdvantageActorCriticAgent(
                policy=_TestPolicy(5),
                value=_TestValue(2),
                predictor_model=_TestModel(2),
                target_model=_TestModel(2),
                optimizer=tf.train.GradientDescentOptimizer(1.),
                predictor_optimizer=tf.train.GradientDescentOptimizer(1.))

            total_loss = agent.compute_loss(
                states=tf.zeros([4, 2, 5], tf.float32),
                actions=tf.zeros([4, 2, 5], tf.float32),
                rewards=tf.ones([4, 2, 2], tf.float32),
    ...
    def testEstimateGradients(self):
        with context.eager_mode():
            ...
            policy_value_grads_and_vars, predictor_grads_and_vars = agent.estimate_gradients(
                states=tf.zeros([4, 2, 5], tf.float32),
                actions=tf.zeros([4, 2, 5], tf.float32),
                rewards=tf.ones([4, 2, 2], tf.float32),
                weights=tf.ones([4, 2], tf.float32))
            ...
            grads, _ = zip(*predictor_grads_and_vars)
            variables = agent.predictor_model.trainable_variables
            for grad, var in zip(grads, variables):
                self.assertShapeEqual(tf.shape(grad).numpy(), tf.shape(var))
            self.assertEqual(5, len(agent.loss))
