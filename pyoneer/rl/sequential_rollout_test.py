...
from pyoneer.rl import sequential_rollout_impl


class SequentialRolloutTest(test.TestCase):

    def test_sequential_rollout_contiguous(self):
        with context.eager_mode():
            ...
            episodes = 5
            max_steps = 100
            rollout = sequential_rollout_impl.sequential_rollout(
                test_env,
                next_action_fn,
                initial_state=None,
                initial_action=None,
    ...
    def test_sequential_rollout_non_contiguous(self):
        with context.eager_mode():
            test_env = test_fixtures.CounterEnv()
            ...
            expected_states = gen_array_ops.tile(math_ops.range(max_steps), [episodes])
            expected_actions = array_ops.zeros_like(expected_states)
            expected_rewards = array_ops.zeros_like(expected_states)
            expected_weights = array_ops.ones_like(expected_states)
    ...
    def test_sequential_rollout_done_on_max_steps(self):
        with context.eager_mode():
            test_env = test_fixtures.CounterEnv()
