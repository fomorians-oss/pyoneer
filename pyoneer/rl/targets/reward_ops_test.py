...
class RewardOpsTest(tf.test.TestCase):
    def testDiscountedReturns(self):
        discount = 0.99
        rewards = tf.constant([[1.0, 1.1, 1.2], [1.0, 1.1, 1.2]], tf.dtypes.float32)
        expected_returns = tf.stack(
            [
                rewards[:, 0]
                + discount * rewards[:, 1]
                + (discount ** 2 * rewards[:, 2]),
    ...
    def testLambdaReturns(self):
        discount = 0.99
        lambdas = 0.975
        rewards = tf.constant([[1.0, 1.1, 1.2], [1.0, 1.1, 1.2]], tf.dtypes.float32)
        values = tf.constant(
            [[9.0, -1.1, 1.022], [8.0, -123.1, 2.1]], tf.dtypes.float32
        )
        values_tp1 = tf.concat([values[:, 1:], tf.zeros_like(values[:, :1])], axis=1)
        lambda_returns = rewards + discount * values_tp1 * (1.0 - lambdas)

        expected_returns = tf.stack(
            [
                lambda_returns[:, 0]
                + (discount * lambdas) * lambda_returns[:, 1]
                + ((discount * lambdas) ** 2 * lambda_returns[:, 2]),
                lambda_returns[:, 1] + (discount * lambdas) * lambda_returns[:, 2],
                lambda_returns[:, 2],
            ],
            axis=1,
        )

        returns = reward_ops.lambda_returns(
            rewards, values, discounts=discount, lambdas=lambdas
        )
        self.assertAllClose(returns, expected_returns, rtol=5e-5, atol=5e-5)
    ...
    def testVTraceReturns(self):
        ...
        c_hat = 1.0
        rewards = tf.constant([[1.0, 1.1, 1.2], [1.0, 1.1, 1.2]], tf.dtypes.float32)
        values = tf.constant([[1.1, 1.2, 1.3], [1.1, 1.2, 1.3]], tf.dtypes.float32)
        probs = tf.constant([[0.2, 0.3, 0.4], [0.2, 0.3, 0.4]], tf.dtypes.float32)
        values_tp1 = tf.concat([values[:, 1:], tf.zeros_like(values[:, :1])], axis=1)

        probs_old = probs
        rhos = tf.minimum(rho_hat, probs / probs_old)
        cs = tf.minimum(c_hat, probs / probs_old)
        td = rhos * ((rewards + discount * values_tp1) - values)
        expected_returns = tf.stack(
            [
                (
                    (td[:, 0] * cs[:, 0])
                    + discount * (cs[:, 1] * cs[:, 0] * td[:, 1])
                    + discount ** 2 * (cs[:, 2] * cs[:, 1] * cs[:, 0] * td[:, 2])
                ),
                ((td[:, 1] * cs[:, 1]) + discount * (cs[:, 2] * cs[:, 1] * td[:, 2])),
                td[:, 2] * cs[:, 2],
            ],
            axis=1,
        )
        ...
        v_trace_values = reward_ops.v_trace_returns(
            ...
            values,
            tf.math.log(probs),
            tf.math.log(probs_old),
            discounts=discount,
        )
        self.assertAllClose(
            v_trace_values, expected_v_trace_values, rtol=5e-5, atol=5e-5
        )
    ...
    def testGeneralizedAdvantageEstimation(self):
        ...
        td = (rewards + discount * values_tp1) - values
        expected_generalized_td = tf.stack(
            [
                (
                    td[:, 0]
                    + (discount * lambdas) * (td[:, 1])
