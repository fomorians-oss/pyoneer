...
class Env(object):
    ...
    def __init__(self, env):
        ...
        # Specs.
        state_spec = _space_to_spec(env.observation_space, _Counter(), name='State')
        next_state_spec = _space_to_spec(env.observation_space, _Counter(), name='NextState')
        action_spec = _space_to_spec(env.action_space, _Counter(), name='Action')
        if hasattr(env, 'reward_space'):
            reward_spec = _space_to_spec(env.reward_space, _Counter(), name='Reward')
        else:
            reward_spec = tf.TensorSpec([], tf.dtypes.float32, name='Reward')
        terminal_spec = tf.TensorSpec([], tf.dtypes.bool, name='Terminal')
        weight_spec = tf.TensorSpec([], tf.dtypes.float32, name='Weight')
        ...
        (self._initial_reward,
         ...
         self._initial_weight) = debugging_ops.mock_spec(
            ...
            (reward_spec, state_spec, terminal_spec,
             weight_spec),
            initializers=(
                tf.zeros, tf.zeros, tf.zeros, tf.ones))

        # Meta information.
        self.output_specs = unroll_ops.Transition(
            state=state_spec,
            reward=reward_spec,
            next_state=next_state_spec,
    ...
    @staticmethod
    def from_gym_spec(env_spec, batch_size=None):
        def make_fn():
        ...
        if batch_size is None:
            ...
            gym_env = batch_impl.Batch(make_fn, batch_size)

        # Wrap it in a Model.
        env_model = Env(gym_env)
        return env_model
    ...
    def step(self, agent_outputs, env_outputs):
        ...
        weight = tf.cast(
            ...
            self.output_specs.weight.dtype)
