...
class BatchEnv(gym.Env):
    ...
    def __init__(self, spec):
        """Create a new batched gym.Env interface from the given `str` spec.

        Args:
          spec: gym.Env spec str.
        """
        if callable(spec):
            self.make_and_seed = spec
            env = spec()
        else:
            self.make_and_seed = make_and_seed(spec)
            env = gym.make(spec)

        self.observation_space = env.observation_space
        self.action_space = env.action_space
        self.__seed__ = None
        self.__cache__ = cache([env])
    ...
    def reset(self, episodes=1):
        ...
        if episodes != len(self.__cache__.items):
            self.__cache__.fix(
                lambda: self.make_and_seed(self.__seed__),
