...
def symmetric_mean_absolute_percentage_error(y_true,
                                             y_pred,
                                             sample_weight=None,
                                             multioutput='raw_values'):
...
class SymmetricMeanAbsolutePercentageError(metrics.Mean):
    ...
    def call(self, labels, predictions, weights=None):
        ...
        check_ops.assert_equal(
            ...
            array_ops.shape(predictions),
            message="Shapes of labels and predictions are unequal")
        sapes = 2 * tf.abs(predictions - labels) / (
            tf.abs(predictions) + tf.abs(labels))
        super(SymmetricMeanAbsolutePercentageError, self).call(
            sapes, weights=weights)
        if weights is None:
            return labels, predictions
        else:
            return labels, predictions, weights
