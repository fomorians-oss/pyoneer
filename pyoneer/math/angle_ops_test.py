...
class AnglesOpsTest(test.TestCase):
    ...
    def test_to_cartesian(self):
        with context.eager_mode():
            ...
            expected_x = tf.constant(
                [+1, -1, +0, +1, +0, -1, +1], dtype=tf.float32)
            expected_y = tf.constant(
                [+0, +0, -1, +0, +1, +0, +0], dtype=tf.float32)
            self.assertAllClose(outputs_x, expected_x)
            self.assertAllClose(outputs_y, expected_y)

    def test_to_polar(self):
        with context.eager_mode():
            inputs_x = tf.constant(
                [+1, -1, +0, +1, +0, -1, +1], dtype=tf.float32)
            inputs_y = tf.constant(
                [+0, +0, -1, +0, +1, +0, +0], dtype=tf.float32)
            ...
            expected_phi = tf.constant(
                [0, math.pi, -math.pi / 2, 0, math.pi / 2, math.pi, 0],
                dtype=tf.float32)
