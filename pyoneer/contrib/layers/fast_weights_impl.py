...
from pyoneer.contrib.layers import layer_normalization
...
class FastWeightsRNNCell(rnn_cell_impl.LayerRNNCell):
  ...
  def call(self, inputs, state, training=False):
    ...
    h = self._activation(
        layer_normalization.layer_norm(inner, self._inner_beta, self._inner_gamma)
...
class FastWeightsLSTMCell(rnn_cell_impl.LayerRNNCell):
  ...
  def call(self, inputs, state, training=False):
    ...
    if self._use_layer_norm:
      gate_inputs = layer_normalization.layer_norm(gate_inputs, self._gate_beta, self._gate_gamma)
    ...
    if self._use_layer_norm:
      new_c = layer_normalization.layer_norm(new_c, self._c_beta, self._c_gamma)
